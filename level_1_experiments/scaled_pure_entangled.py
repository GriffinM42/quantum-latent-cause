...
esti_state[0][0] = 0.5
esti_state[0][dx*dy-1] = 0.5
esti_state[dx*dy-1][0] = 0.5
...
tolerance = ih.tolerance + (max(math.log2(dx), math.log2(dy))+1) * ih.smoothing
entrop_thresh = ih.entrop_thresh
extern_thresh = ih.extern_thresh
dep_gate = ih.dep_gate + (max(math.log2(dx), math.log2(dy))+1) * ih.smoothing
